...
def parse_m3u_file(m3u_file_path, clean_group_title, process_value, REPLACE_TERMS, REPLACE_DEFAULTS, SCRUB_HEADER, SCRUB_DEFAULTS, REMOVE_TERMS, REMOVE_DEFAULTS):
    try:
        ...
        if line.startswith("#EXTINF"):
            try:
                ...
                if 'group-title' in key_value_pairs:
                    key_value_pairs['group-title'] = process_value(key_value_pairs['group-title'], remove_header=SCRUB_DEFAULTS)
                    key_value_pairs['group-title'] = process_value(key_value_pairs['group-title'], remove_header=SCRUB_HEADER)
                    key_value_pairs['group-title'] = process_value(key_value_pairs['group-title'], replace=REPLACE_DEFAULTS)
                    key_value_pairs['group-title'] = process_value(key_value_pairs['group-title'], replace=REPLACE_TERMS)

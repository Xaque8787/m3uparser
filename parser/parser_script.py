...
from utils import *
from config import (initialize_vars, dirmake, create_vars, process_env_variable,
                    str_to_bool, process_env_special, vars_position)
...
def main():
    try:
        ...
        sync_directories(*vars_position(process_env_variable, str_to_bool, process_env_special,
                                        'master_mov_dir', 'local_mov_dir'))
        sync_directories(*vars_position(process_env_variable, str_to_bool, process_env_special,
                                        'master_tv_dir', 'local_tv_dir'))
        ...
        if init_var['live_tv'] is True:
            move_files(*vars_position(process_env_variable, str_to_bool, process_env_special, 'live_tv_file',
                                     'live_tv_dir'))
        # Count .strm files and live TV channels
        [livetv_channels_count, movie_strm_count, tv_strm_count, unsorted_strm_count] = count_emup(live_tv_entries,
                                                                                                   *vars_position(process_env_variable, str_to_bool,
                                                                                                                  process_env_special, 'tv_dir',
                                                                                                                  'movies_dir', 'unsorted_dir'))
